...
export default class GSUBProcessor extends OTProcessor {
    applyLookup(lookupType: any, table: any): boolean {
        switch (lookupType) {
            case 1: {
                // Single Substitution
            ...
            case 2: {
                ...
                if (index !== -1) {
                    ...
                    let replacement = sequence.slice(1).map((gid: any, i: any) => {
            ...
            case 4: {
                ...
                for (let ligature of table.ligatureSets.get(index)) {
                    let matched = this.sequenceMatchIndices(1, ligature.components) as any[]
                    if (!matched) {
                        continue
                    }
                        ...
                        if (isMarkLigature) {
                            ...
                            while (idx < matchIndex) {
                                var ligatureComponent =
                                    curComps -
                                    lastNumComps +
                                    Math.min(this.glyphs[idx].ligatureComponent || 1, lastNumComps)
