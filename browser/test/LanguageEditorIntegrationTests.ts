...
test("editor language integration shows hover and definition", async (t) => {
    ...
    t.is(mockDefinitionRequestor.pendingCallCount, 1)
    t.is(mockHoverRequestor.pendingCallCount, 1)
    ...
    t.is(showDefinitionCount, 1, "Definition was shown")
    t.is(showHoverCount, 1, "Hover was shown")
})

test("respects editor.quickInfo.delay setting for hover", (t) => {
    ...
    t.is(mockHoverRequestor.pendingCallCount, 0, "Should be no request queued yet...")

    // Tick just before the delay....
    clock.tick(499)

    t.is(mockHoverRequestor.pendingCallCount, 0, "Should be no request queued, right before the time limit..")

    // Tick just after the delay...
    clock.tick(2)

    // There should now be a request queued up
    t.is(mockHoverRequestor.pendingCallCount, 1, "Should now be a request pending, because we've exceeded the limit")
})

test("doesn't show slow hover response that completes after cursor moves", async (t) => {
    mockEditor.simulateModeChange("normal")
    mockEditor.simulateBufferEnter(new Mocks.MockBuffer())
    mockEditor.simulateCursorMoved(1, 1)
    ...
    t.is(mockHoverRequestor.pendingCallCount, 1, "Verify we have a request queued up")
    ...
    t.is(hoverShowCount, 0, "Hover should never be shown, because the cursor moved.")
})

test("doesn't show slow hover response that completes after mode changes", async (t) => {
    mockEditor.simulateModeChange("normal")
    mockEditor.simulateBufferEnter(new Mocks.MockBuffer())
    mockEditor.simulateCursorMoved(1, 1)
...
test("editor language integration hides hover on mode change", async (t) => {
    let showHoverCount = 0
    ...
    t.is(showHoverCount, 1, "Hover was shown")
    t.is(hideHoverCount, 0, "No calls to hide hover yet")

    mockEditor.simulateModeChange("cmdline")

    clock.runAll()

    t.is(showHoverCount, 1, "Hover show count should be unchanged")
    t.is(hideHoverCount, 1, "There should now be a call to hide the hover")
})

test("#1063 - editor language integration doesn't show hover if there are no results", async (t) => {
    let showDefinitionCount = 0
    ...
    t.is(showDefinitionCount, 0, "Definition should not be shown")
})
