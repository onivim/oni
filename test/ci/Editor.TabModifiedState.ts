...
export const test = async (oni: Oni.Plugin.Api) => {
    ...
    assert.strictEqual(modifiedIconStyle.opacity, "0", "Check tab has no modified icon")
    ...
    assert.strictEqual(modifiedIconStyle.opacity, "1", "Check tab now has a modified icon")
    ...
    assert.strictEqual(
        ...
        "1",
        "Check tab has modified icon after opening split",
    )
    ...
    assert.strictEqual(
        modifiedIconStyle.opacity,
        "1",
        "Check tab still has modified icon after swapping",
    )
}
