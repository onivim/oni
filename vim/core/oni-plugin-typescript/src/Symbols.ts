...
export const getWorkspaceSymbols = (oni: Oni.Plugin.Api, host: TypeScriptServerHost) => async (message: string, payload: ISymbolSearchParams): Promise<types.SymbolInformation[]> => {
    ...
    const convertNavtoItemToSymbolInformation = (item: protocol.NavtoItem): types.SymbolInformation => {
        return {
            name: item.name,
...
export const getDocumentSymbols = (oni: Oni.Plugin.Api, host: TypeScriptServerHost) => async (message: string, payload: ITextDocumentParams): Promise<types.SymbolInformation[]> => {
    ...
    const appendItemToResults = (item: protocol.NavigationTree, results: types.SymbolInformation[]) => {
